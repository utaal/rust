...
use crate::def::{
    ...
    suffix_local_id, suffix_typ_param_id, prefix_recursive, SnapPos, Spanned, FUEL_BOOL, FUEL_BOOL_DEFAULT,
...
pub(crate) fn exp_to_expr(ctx: &Ctx, exp: &Exp) -> Expr {
    match &exp.x {
        ...
        ExpX::Call(rec, x, typs, args) => {
            ...
            let name = suffix_global_id(&if *rec {
                prefix_recursive(&air_ident)
            } else { air_ident });
    ...
    match &stm.x {
        ...
        StmX::Fuel(x, fuel) => {
            ...
            if *fuel >= 2 {
                ...
                let eq = mk_eq(&ident_var(&crate::def::prefix_fuel_nat(&path_to_air_ident(&x))), &added_fuel);
