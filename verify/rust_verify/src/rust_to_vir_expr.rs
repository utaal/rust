    ...
    match &expr.kind {
        ...
        ExprKind::Call(fun, args_slice) => {
            match fun.kind {
                ...
                )) => fn_call_to_vir(bctx, expr, expr_to_function(fun), bctx.types.node_type(fun.hir_id), bctx.types.node_substs(fun.hir_id), fun.span, args_slice),
        ...
        ExprKind::MethodCall(_name_and_generics, _call_span_0, all_args, _call_span_1) => {
            let fn_def_id = bctx.types.type_dependent_def_id(expr.hir_id).expect("def id of the method definition");
            let sig = if let rustc_hir::Node::ImplItem(rustc_hir::ImplItem { kind: rustc_hir::ImplItemKind::Fn(sig, _body_id), .. }) = tcx.hir().get_if_local(fn_def_id).expect("fn def for method in hir") {
                sig
            } else {
                panic!("unexpected hir for method impl item");
            };
            fn_call_to_vir(bctx, expr, fn_def_id, tcx.type_of(fn_def_id), bctx.types.node_substs(expr.hir_id), sig.span, all_args)
        }
        _ => {
